...
function connectDB() {
	// 데이터베이스 연결 정보
	var databaseUrl = 'mongodb+srv://myuserdb:users3@port.eoxeaws.mongodb.net/myuserdb';
...
router.route('/public/login.html').post(function (req, res) {
	...
	if (database) {
		authUser(database, paramId, paramPassword, function (err, docs) {
			...
			if (docs) {
				...
				res.redirect('/public/login_success.html')
				// res.writeHead('200', {'Content-Type':'text/html;charset=utf8'});
...
router.route('/public/adduser.html').post(function (req, res) {
	...
	if (database) {
		addUser(database, paramId, paramPassword, paramName, function (err, addedUser) {
			...
			if (addedUser) {
				...
				res.redirect('/public/adduser_success.html');
				// res.writeHead('200', {'Content-Type':'text/html;charset=utf8'});
...
//사용자 리스트 함수
router.route('/public/listuser.html').post(function (req, res) {
	...
	if (database) {
		...
		UserModel.findAll(function (err, results) {
			...
			if (results) {  // 결과 객체 있으면 리스트 전송
				...
				res.write('</ul></div>');
				res.write('<br><br><button><a href="/public/adduser.html">사용자추가</a>');
				res.write('<button><a href="/public/login.html">로그인</a>')
				res.end();
			} else {  // 결과 객체가 없으면 실패 응답 전송
				res.writeHead('200', { 'Content-Type': 'text/html;charset=utf8' });
